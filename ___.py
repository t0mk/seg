...
class SampleSet:
    ...
    def getFiltered(self, zmax):
        xa, ya = self.getRawAvg()
        d = np.array([disterr(t.xs, t.ys, xa, ya) for t in self.trajs])
        out = [self.trajs[i] for i in np.where(d>zmax)[0]]
        ix = (d<zmax)
        self.d = d

        filtix = np.where(ix)[0]

        filtered = [self.trajs[i] for i in filtix]
        return filtered, out

    def getAvg(self, zmax, eps):
        self.endpoints()
        if len(self.trajs) > 3:
            self.filtered, self.out = self.getFiltered(zmax)
        else:
            self.filtered, self.out = self.trajs, []
        ...
        if len(self.filtered)<2:
            ...
            self.out = [self.trajs[i] for i in distrank[nbest:]]
    ...
    def pax(self, ax, lims, eps):
        ...
        if self.xp is not None:
            ax.set_xlabel("#%d err:%.3f eps:%.3f, ns: %d, out: %d" % 
                          (self.n, self.err, eps, len(self.filtered), len(self.out)))
        #for t in self.trajs:    
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        ...
        sys.exit(1)
    eps = .126
    r = getResults(sys.argv[1], -1, 1, eps)
    print(r)
